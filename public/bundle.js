      ...
      return function (ctx) {
          return function (stateRef) {
              return function __do() {
                  ...
                  Effect.foreachE(v.positions)(function (pos) {
  ...
  var accel = function (v) {
      return function (radius) {
          return function (v1) {
              ...
              var accelY = scalar * v1.y;
              var accelX = scalar * v1.x;
              return {
                  x: accelX,
                  y: accelY
              };
      ...
      return function (dims) {
          return function (ctx) {
              return function (stateRef) {
                  return function __do() {
                      var v = Effect_Ref.read(stateRef)();
                      var radius = hypotenuse(v.pos);
                      var unitVect = {
                          x: v.pos.x / radius,
                          y: v.pos.y / radius
                      };
                      var currAccel = accel(v.masses)(radius)(unitVect);
                      var newPos = verletPos(v.pos)(v.vel)(currAccel)(consts.timeStep);
                      var newRadius = hypotenuse(newPos);
                      var newUnitVect = {
                          x: newPos.x / newRadius,
                          y: newPos.y / newRadius
                      };
                      var newAccel = accel(v.masses)(newRadius)(newUnitVect);
                      var newVel = verletVel(v.vel)(currAccel)(newAccel)(consts.timeStep);
                      var a2 = v.masses.m2 / v.masses.total;
                      var a1 = v.masses.m1 / v.masses.total;
                      var newState = {
                          pos: newPos,
                          vel: newVel,
                          masses: v.masses,
  ...
  var main = function __do() {
      ...
      if (v instanceof Data_Maybe.Just) {
          ...
          var state = {
              ...
              vel: {
                  x: 0.0,
                  y: initialVelocity(1.0)(consts.eccentricity)
              },
              masses: {
                  m1: 1.0,
                  m2: consts.massRatio,
      ...
      throw new Error("Failed pattern match at Main (line 66, column 3 - line 93, column 72): " + [ v.constructor.name ]);
  };
